...
!
      ...
      use ice_colpkg_tracers, only: bio_index_o
...
! author: Elizabeth C. Hunke, LANL
      ...
      use ice_colpkg_shared, only: nit_data_type, sil_data_type, bgc_data_dir, &
          max_algae, max_doc, max_dic, restore_bgc
      use ice_colpkg_tracers, only: tr_bgc_Sil, tr_bgc_Nit
...
! authors: Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared, only: fe_data_type, bgc_data_dir
...
! authors: Elizabeth Hunke, LANL
      ...
      use ice_read_write, only: ice_open_nc, ice_read_nc, ice_close_nc
      use ice_communicate, only: my_task, master_task
      use ice_constants_colpkg, only: nspint, c0
      use ice_colpkg_shared, only: max_aero, modal_aero
